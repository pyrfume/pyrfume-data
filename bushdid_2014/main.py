...
s1 = s1.join(cids, on='C.A.S.').set_index('CID').sort_index()
...
behavior = pd.melt(s2, id_vars=['Test UID'],
                   value_vars=['subject %d' % i for i in range(1, 27)],
                   var_name='Subject', value_name='Correct')

behavior = behavior.replace({'subject %d' % i: i for i in range(1, 27)})
behavior.rename(columns={'Test UID': 'Stimulus'}, inplace=True)
behavior = behavior.dropna().set_index(['Stimulus', 'Subject']).sort_index()
...
mixtures.rename(columns={'Test UID': 'Stimulus'}, inplace=True)
mixtures = mixtures.set_index(['Stimulus', 'Answer']).sort_index()
